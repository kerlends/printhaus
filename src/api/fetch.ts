...
const shopName = process.env.GATSBY_SHOPIFY_SHOP_NAME as string;
const storefrontToken = process.env.GATSBY_SHOPIFY_ACCESS_TOKEN as string;
...
const shopifyRequest = async ({ query, variables }: ShopifyRequestOptions) => {
  const options = {
    method: 'post',
    headers: {
...
export const removeLineItems = async (
  ...
  const { checkoutLineItemsRemove: data } = await shopifyRequest({
...
export const updateLineItems = async (
  ...
  const { checkoutLineItemsUpdate: data } = await shopifyRequest({
