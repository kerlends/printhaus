...
class ProductVariantSelect extends React.Component<Props, State> {

  state = {
    activeVariantId: this.props.variants[0].id
  };

  handleVariantChange = (evt: React.SyntheticEvent<HTMLSelectElement>) => this.setState({
  ...
  render() {
    ...
    return <div className={classes.container}>
        <label className={classes.select} htmlFor="variantSelect">
          <ProductVariant {...activeVariant} />
        </label>
        <select id="variantSelect" value={activeVariant.id} onChange={this.handleVariantChange}>
